...
public class GithubOAuthProvider {
    ...
    public static String getAccessTokenByCodeAndUrl(String code, String url) throws WTException {
        ...
        if (Objects.isNull(result) || StringUtils.isBlank(result.getString("access_token"))) {
            throw new WTException("登录失败");
        }
    ...
    public static synchronized String buildCommonRequestTOPost(String path, String token, boolean isSendUserInfo,
        ...
        logger.debug("GithubOAuthProvider.buildCommonRequest INTO " + LocalDateTime.now());
        ...
        try {
            ...
            if (statusCode != 200) {
                throw new WTException("登录失败查看信息: " + responseString);
            }
        } catch (Exception e) {
            logger.error("buildCommonRequestTOPost ", e);
        } finally {
            try {
                if (response != null) {
                    response.close();
        ...
        logger.debug("GithubOAuthProvider.buildCommonRequest END " + LocalDateTime.now());
    ...
    public static synchronized String buildCommonRequestTOGet(String path, String token) {
        logger.debug("GithubOAuthProvider.buildCommonRequestTOGet INTO " + LocalDateTime.now());
        String responseString = null;
        CloseableHttpClient httpclient = null;
        CloseableHttpResponse response = null;
        try {
            ...
            logger.error("buildCommonRequestTOGet error", e);
        } finally {
            try {
                if (response != null) {
                    response.close();
        ...
        logger.debug("GithubOAuthProvider.buildCommonRequestTOGet END " + LocalDateTime.now());
        return responseString;
    }
}
